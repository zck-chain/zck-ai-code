...
import com.zck.aicodemother.langgraph4j.ai.CodeQualityCheckService;
...
import com.zck.aicodemother.utils.ReadFileUtil;
        ...
        return node_async(state->{
            ...
            try{
                ...
                if (StrUtil.isBlank(codeContent)) {
                    log.warn("未找到拼接代码文件内容");
                    qualityResult=QualityResult.builder()
                            .isValid(false)
                            .errors(List.of("未找到可检查的代码文件"))
                            .suggestions(List.of("请确认代码生成成功"))
                            .build();
                }else{
                    //2.调用代码质量检查ai进行代码质量检查
